...
public class DateTimeUtil {
	 ...
	 */
	    ...
	    public static String formatInterval(double interval, int count, Locale locale) {
	     ...
	     //   Map<String, Object> uiDateTimeMap = PropertiesUtil.getResourceBundleMap("DateTimeLabels", locale);
	        ...
	        for (int i = parts.size() - 1; i >= 0 && count > 0; i--) {
	            ...
	            Object label;
	            if (D.intValue() == 1) {
	               label = null;//uiDateTimeMap.get(timevals[i][1] + ".singular");
	            } else {
	                label = null;//uiDateTimeMap.get(timevals[i][1] + ".plural");
	            }
	            sb.append(label);
	        }
	        return sb.toString();
	    ...
	    public static int getSecond(Timestamp stamp, TimeZone timeZone, Locale locale) {
	        Calendar cal = DateTimeUtil.toCalendar(stamp, timeZone, locale);
